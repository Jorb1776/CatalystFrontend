    ...
    if (productId) {
      axios
        ...
        .then((r) => {
          ...
          // Set mold search to the base number if mold is selected
          if (p.moldInsert?.baseNumber) {
            setMoldSearch(p.moldInsert.baseNumber);
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (productId) {
        ...
        console.log("[ProductForm] Create response:", response);
      ...
      console.error("[ProductForm] Error status:", error?.response?.status);
      toast.error(
        `Save failed: ${
    ...
    <form onSubmit={handleSubmit} style={formStyle}>
      <div style={gridContainer}>
        ...
        <div style={field}>
          ...
          <select
            value={form.colorantId}
            onChange={(e) => setForm({ ...form, colorantId: e.target.value })}
            style={selectStyle}
          >
        ...
        <div style={{ ...field, position: "relative" }}>
          <label>Mold Base</label>
          <input
            value={moldSearch}
            onChange={(e) => {
              setMoldSearch(e.target.value);
              setMoldDropdownOpen(true);
              // Clear selection if user modifies search
              if (form.moldId) setForm({ ...form, moldId: "" });
            }}
            onFocus={() => setMoldDropdownOpen(true)}
          ...
          {moldDropdownOpen && (
            <>
              <div
                ...
                onClick={() => setMoldDropdownOpen(false)}
              />
              <div style={dropdownStyle}>
                ...
                {molds
                  .filter((m) =>
                    m.baseNumber
                      .toLowerCase()
                      .includes(moldSearch.toLowerCase())
                  )
                  .slice(0, 10)
                  .map((m) => (
                    <div
                      ...
                      style={dropdownItemStyle}
                      onMouseEnter={(e) =>
                        (e.currentTarget.style.background = "#1a3d1a")
                      }
                      onMouseLeave={(e) =>
                        (e.currentTarget.style.background = "transparent")
                      }
                      onClick={() => {
                        setForm({ ...form, moldId: m.moldID.toString() });
                        setMoldSearch(m.baseNumber);
                        setMoldDropdownOpen(false);
                      }}
                    >
                      {m.baseNumber}
                ...
                {molds.filter((m) =>
                  m.baseNumber.toLowerCase().includes(moldSearch.toLowerCase())
                ).length === 0 && (
                  <div
                    style={{
                      ...dropdownItemStyle,
        ...
        <div style={field}>
          <label>Full Box Qty</label>
          <input
            type="number"
            value={form.fullBoxQty}
            onChange={(e) => setForm({ ...form, fullBoxQty: e.target.value })}
            style={inputStyle}
          />
        </div>
            ...
            <div key={a.additiveID} style={additiveRow}>
              <label style={checkboxLabel}>
                <input
                  ...
                  onChange={(e) => {
                    if (!e.target.checked)
                      setSelectedAdditives((p) =>
                        p.filter((x) => x.id !== a.additiveID)
                      );
                    else
                      setSelectedAdditives((p) => [
                        ...p,
                        { id: a.additiveID, pct: "0" },
                      ]);
                  }}
                />
                {a.name}
...
const additivesSection: React.CSSProperties = {
  marginTop: 24,
  padding: 16,
  background: "#1a1a1a",
  borderRadius: 8,
...
const additiveRow: React.CSSProperties = {
  display: "flex",
  alignItems: "center",
  gap: 12,
  margin: "8px 0",
...
const ldrTag: React.CSSProperties = {
  color: "#0f0",
  background: "#1a3d1a",
  padding: "3px 8px",
  borderRadius: 6,
