...
const ScheduleForm = () => {
  ...
  const submit = async (e) => {
    ...
    const url = id 
      ? `/api/productionschedules/${id}`
      : '/api/productionschedules';
      ...
      <form onSubmit={submit}>
        <div style={{ marginBottom: 10 }}>
          ...
          >
            ...
            {orders.map(schedule => (
              <option key={schedule.prodOrderID} value={schedule.prodOrderID}>
                {schedule.product?.name} (Qty: {schedule.quantity})
              </option>
            ))}
        </select>
        </div>
